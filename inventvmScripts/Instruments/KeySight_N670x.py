...
class N670x:
    ...
    def arb_Trapezoid__Current(self,channel:int,initial_Current:float,end_Current:float,initial_Time:float,raise_Time:float,top_Time:float,fall_Time:float,end_Time:float):
        self.my_instr.write(f'ARB:CURR:TRAP:STAR  {str(initial_Current)},(@{str(channel)})')
        self.my_instr.write(f'ARB:CURR:TRAP:END  {str(end_Current)},(@{str(channel)})')
