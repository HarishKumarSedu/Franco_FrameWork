...
class Efficiency:
    ...
    def __init__(self,dut) -> None:
        ...
        self.phase_Effieciency_Bat(phase=0,phase_thld=1,No_phase=3,vbus=20.0,loop_current_limit=-15.1,sheet='112_ph1234_vbus20V_25C_1')
    ...
    def phase_Effieciency_Bat(self,phase=0,phase_thld=0,vbus=5.0,No_phase=1,loop_current_limit=-3.005,sheet='ph1_vbat_4V_vbus_5V',):
        print(sheet)
        # self.supply.outp_OFF(channel=1)
        # self.startup.buck_ClosedLoop(vbat=4,ibat=16,icmd_ph=6,No_phase=1,ibus=3.3,phase=1)
        voltage=3.99
        ...
        sleep(1)
        ibat_set = 0
        self.Bat.setVoltage(voltage=3.98)
        self.Bat.outp_ON()
        sleep(0.1)
        vbat_V = []
        vbus_V = []
        ...
        try :
            ...
            if self.Bat.getCurrent() < -0.01:
                ...
                while self.Bat.getCurrent() >= loop_current_limit:
                    ...
                    while (sum([abs(self.Bat.getCurrent()) for _ in range(0,4)])/4) <= ibat_set: 
                        sleep(0.005)
                        voltage = voltage - 0.0001
                    # self.Bat.setCurrent(current=-ibat_set)
                        self.Bat.setVoltage(voltage=voltage)
                    vbat_V.append(self.multimeter.meas_V())
                ...
                writeInExcel(sheet=sheet,filename='effieciency\effiecincy_New.xlsx',vbat_V=vbat_V,ibat_A=ibat_A,vbus_V=vbus_V,ibus_A=ibus_A,efficiency=efficiency)
                input()
                while voltage <= 3.98:
                    voltage = voltage + 0.001
                    self.Bat.setVoltage(voltage=voltage)
