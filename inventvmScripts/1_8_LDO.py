...
class LDO_1_8:
    ...
    def __init__(self) -> None:
        ...
        # self.ldo_1_8_loading(sheet='Vbat_3_8V_vbus_5v')
        self.supply.setVoltage(3,0) # Vbat 
        self.supply.setVoltage(1,5) # vbus 
        self.ldo_1_8_loading(filename='Vbat_0V_vbus_5v')

    def ldo_1_8_loading(self,filename):
        ldo_1_8_current = []
        ldo_1_8_voltage = []
        for i in range(-5,-55,-5):
            ldo_1_8_current.append(i*0.001)
            self.supply.setNegCurrent(channel=4,current=ldo_1_8_current[-1])
            sleep(0.1)
            ldo_1_8_voltage.append(self.voltMeter.meas_V())
        
        print(ldo_1_8_current)
        print(ldo_1_8_voltage)

        self.writeInExcel(ldo_1_8_voltage,ldo_1_8_current,sheet=filename)
    ...
    def writeInExcel(self,x,y,filename='sheet1',**args):
        df = pd.DataFrame({
                   '1.8 LDO Voltage (V)': x,
                   'Meas Current': y,
                        })

        writer = pd.ExcelWriter(f'LDO_1_8V/LDO_1_8V_{filename}.xlsx', engine='xlsxwriter')
