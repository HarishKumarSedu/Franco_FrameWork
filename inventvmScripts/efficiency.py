...
class Efficiency:
    ...
    def phase_Effieciency_1(self,phase=0,phase_thld=0,vbus=5.0,No_phase=1,loop_current_limit=-3.005,sheet='ph1_vbat_4V_vbus_5V',):
        ...
        self.startup.buck_ClosedLoop(vbat=4,ibat=14.0,No_phase=No_phase,ibus=3.3,icmd_ph=6.0,phase=phase)
        self.dut.IVM.REG_DRV_INDCS_RW.DS_DRV_SLEW.value = 1
        if vbus > 8 :
            self.dut.IVM.REG_DRV_INDCS_RW.DS_DRV_SLEW.value = 3
        ibat_set = 0
        ...
        try :
            ...
            if self.supply.getCurrent(channel=3) < -0.01:
                # while self.Bat.getCurrent() > loop_current_limit:
                # while self.supply.getCurrent(channel=3) > loop_current_limit:
                while self.supply.getCurrent(channel=3) > -2.99:
                    # vbat_V.append(self.supply.getVoltage(channel=1))
                    # vbat_V.append(self.voltmeter.meas_V())
                    vbat_V.append(self.supply.getVoltage(channel=3))
                    # ibat_A.append(abs(self.Bat.getCurrent()))
                    ibat_A.append(abs(self.supply.getCurrent(channel=3)))
                    vbus_V.append(self.supply.getVoltage(channel=1))
                    ibus_A.append(self.supply.getCurrent(channel=1))
                    efficiency.append(((vbat_V[-1]*ibat_A[-1])/(vbus_V[-1]*ibus_A[-1]))*100)
                    ibat_set = ibat_set + 0.1
                    # self.Bat.setCurrent(current=-ibat_set)
                    self.supply.setCurrent(channel=3,current=-ibat_set)
                    sleep(0.1)
                ...
                self.dut.block_apis.SIMULINK_MODEL.set_standby_en(1)
                writeInExcel(sheet=sheet,filename='effieciency\effiecincy_2.xlsx',vbat_V=vbat_V,ibat_A=ibat_A,vbus_V=vbus_V,ibus_A=ibus_A,efficiency=efficiency)
        except KeyboardInterrupt:
            self.supply.setCurrent(channel=3,current=-0.1)
