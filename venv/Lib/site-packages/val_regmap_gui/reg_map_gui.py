...
from val_regmap_gui.data_view_model import RegMapModel
...
class RegMapGui(wx.Frame):
    def __init__(self, test_station, caller):
        ...
        self.print_reads = False  # determines if reads are printed to the console
        self.current_reg_obj = None  # keeps track of the current register being edited for write_optmized
        self.write_optimized = False
        self.recent_reg_dict = {}  # {component_obj: [register/field paths]} used to populate the search resuls when search term is empty
        self.recent_reg_limit = 30  # limit for the number of items in the list
        self.clear_recent_text = 'Clear recent items...'  # single-source the text used to clear recent items
        
        # Column Numbers - keep track of the column numbers
        self.name_col_index = 0
        self.addr_col_index = 1
        self.len_col_index = 2
        self.bits_col_index = 3
        self.access_col_index = 4
        self.default_col_index = 5
        self.value_col_index = 6
        self.volatile_col_index = 7
        ...
        self.print_reads_menu_item = wx.MenuItem(self.settings_menu, text='Print reads', kind=wx.ITEM_CHECK, id=wx.ID_ANY)
        self.settings_menu.Append(self.print_reads_menu_item)
        ...
        self.reg_map_dict = {}
        for comp, obj in self.component_dict.items():  # create a dvc for the reg map of each component
            ...
            c0 = dv.DataViewColumn("NAME", tr, self.name_col_index, width=400)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c0)
            c1 = dv.DataViewColumn("ADDR", tr, self.addr_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c1)
            c2 = dv.DataViewColumn("LENGTH", tr, self.len_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c2)
            c3 = dv.DataViewColumn("BITS", tr, self.bits_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c3)
            c4 = dv.DataViewColumn("ACCESS", tr, self.access_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c4)
            c5 = dv.DataViewColumn("DEFAULT", tr, self.default_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c5)
            c6 = dv.DataViewColumn("VALUE", tr_edit, self.value_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c6)
            c7 = dv.DataViewColumn("VOLATILE", tr, self.volatile_col_index, width=80)  # , flags=wx.dataview.DATAVIEW_COL_SORTABLE)
            view.AppendColumn(c7)
            model = RegMapModel(caller=self, component=self.component_dict[comp])  # create a model for the component's reg map
            ...
            self.recent_reg_dict[obj] = [self.clear_recent_text]  # initialize an empty list for the component
    ...
    def menu_event(self, event):
        if event.GetEventObject().FindItem(event.GetId())[1] == self.host_controller_menu:  # user selected a host controller
            comp_path = self.get_objects_component(self.current_component)
            print(f'{comp_path}.active_controller = \'{self.menu_bar.GetLabel(event.GetId())}\'')
            ...
            comp_path = self.get_objects_component(self.current_component)
            print(f'{comp_path}.active_interface = \'{self.menu_bar.GetLabel(event.GetId())}\'')
            ...
            comp_path = self.get_objects_component(self.current_component)
            print(f'{comp_path}.verify_writes = {event.GetEventObject().FindItem(event.GetId())[0].IsChecked()}')
        ...
        elif event.GetEventObject().FindItem(event.GetId())[0] == self.print_reads_menu_item:
            self.print_reads = event.GetEventObject().FindItem(event.GetId())[0].IsChecked()
        ...
        with open(path, 'w', newline='') as report:  # create the report
            ...
            for reg in regs:
                ...
                for field in field_list:
                    ...
                    try:
                        if field.readable:
                            value = self.model.int_to_value(self.model.value_to_int(self.model.GetValue(self.model.ObjectToItem(field), self.value_col_index)))
                        else:
                            value = '0x0'
                    except TypeError:  # we'll get a TypeError if the read failed
                        value = '0x0'
    ...
    def get_search_result(self, event):
        ...
        if self.clear_recent_text in text:
            # clear the recent list
            self.recent_reg_dict[self.current_component] = [self.recent_reg_dict[self.current_component][len(self.recent_reg_dict[self.current_component])-1]]
            self.search_results.SetItems(self.recent_reg_dict[self.current_component])  # set the results value
            self.do_search(None)  # refresh so the new results show
        elif 'show all results...' in text:  # user wants to see all results
            ...
            if 'test_station' in text:  # test station object
                ...
                if text[:2] == '0x':  # user searched an address
                    selection = self.current_component.get_register_by_address(int(text, 16))
    ...
    def add_recent(self, text: str):
        # add an item to the recent results
        if text not in self.recent_reg_dict[self.current_component]:  # only if it doesn't exist
            if self.recent_reg_dict[self.current_component].index(self.clear_recent_text) >= self.recent_reg_limit:  # are we at the max # items?
                # keep items at indices 0 to search_limit-1
                # add the clear recent text at index search_limit
                items_to_keep = self.recent_reg_dict[self.current_component][0:self.recent_reg_limit-1]
                items_to_keep.insert(0, text)  # add the most recent item
                items_to_keep.append(self.clear_recent_text)
                self.recent_reg_dict[self.current_component] = items_to_keep
            else:
                self.recent_reg_dict[self.current_component].insert(0, text)  # add to top of recent items

    def show_results_window(self, event):
        if event.EventType == wx.EVT_SET_FOCUS.typeId:  # user is focused on search box
            self.do_search(None)  # so the results initialize on the first focus when the searchbox is empty
            self.search_results.Show(True)
            self.search_results.GetContainingSizer().Layout()
            if len(self.recent_reg_dict[self.current_component]) > 1:  # if there are recent reg items
                self.search_results.SetItems(self.recent_reg_dict[self.current_component])  # show the list of recent reg items
            event.Skip()
        elif event.EventType == wx.EVT_KILL_FOCUS.typeId:  # user clicked away from search box
    ...
    def do_search(self, event, show_all_results=False):
        ...
        if self.search_results.GetItems():  # searchbox has text
            if self.search_results.GetItems()[0][:2] == '0x':  # user clicked "show all results..." after searching an address
                is_addr = True
        ...
        if is_addr:  # user searched an address
            ...
            except re.error:  # filter out regex errors
                pass
        else:  # user searched a name
            try:  # filter out regex errors
    ...
    def get_key_press(self, event):
        ...
        if self.search_box.HasFocus() and event.GetKeyCode() == 317:
            ...
            # event.Skip() # plays a windows chime so don't skip
    ...
    def update_description(self, event):
        try:  # handle the benign error that occurs when the user collapses all when a field is selected
            ...
            if len(self.get_selected_objects()) == 1 and (isinstance(selection, Field) or isinstance(selection, Register)) and self.is_mini_mode:
                # update the status bar with reg info when in mini mode
                item = self.model.ObjectToItem(selection)
                addr = self.model.GetValue(item, self.addr_col_index)
                length = self.model.GetValue(item, self.len_col_index)
                bits = self.model.GetValue(item, self.bits_col_index)
                access = self.model.GetValue(item, self.access_col_index)
                default = self.model.GetValue(item, self.default_col_index)
                volatile = self.model.GetValue(item, self.volatile_col_index)
                text = f'ADDR: {addr} | LENGTH: {length} | BITS: {bits} | ACCESS: {access} | DEFAULT: {default} | VOLATILE: {volatile}'
        ...
        for reg in reg_dict.keys():
            try:  # in case we get an error
                ...
                for field, value in [(field, field_value_dict[field]) for field in field_value_dict.keys()]:
                    ...
                    self.model.ChangeValue(str(value), field_item, self.value_col_index)  # update the model with the field's calculated value
                    reg_val = reg_val | ((value << field.bit_offset) & field.mask)
                self.model.ChangeValue(str(reg_val), reg_item, self.value_col_index)  # update the model with the reg's calculated value
    ...
    def read(self, obj_list, print_list=None):
        reg_field_dict = {}
        reg_value_dict = {}
        fields = [obj for obj in obj_list if isinstance(obj, Field)]
        regs = [obj for obj in obj_list if isinstance(obj, Register)]
        for obj in obj_list:
            if isinstance(obj, Block):
                regs.extend(self.object_to_reg_list(obj))
        regs = list(set(regs))  # remove any duplicates. This can happen if a block is selected as well as some regs within the block
        # Do field reads
        if len(fields) == 1:
            # get the field's reg
            # get the reg's fields
            # do a read optimized on the fields
            # update the reg and fields
            # print the field read
            # remove the reg from the regs list if it's in there
            try:
                reg_val_dict = {}
                field_val_dict = self.current_component.read_fields_optimized(fields[0].container.get_fields(), reg_val_dict)
                reg_value_dict.update(reg_val_dict)
                reg_field_dict[list(reg_val_dict.keys())[0]] = field_val_dict
                for k, v in reg_val_dict.items():
                    reg_item = self.model.ObjectToItem(k)
                    self.model.ChangeValue(str(v), reg_item, self.value_col_index)  # update the reg in the model
                for k, v in field_val_dict.items():
                    field_item = self.model.ObjectToItem(k)
                    self.model.ChangeValue(str(v), field_item, self.value_col_index)  # update the field in the model
                for reg in reg_val_dict.keys():
                    if reg in regs:
                        regs.remove(reg)
                if print_list is not None:
                    print_list.append(f'{self.get_objects_component(fields[0])}.{fields[0].path}.value')
            except Exception:
                self.indicate_error()
                print(traceback.format_exc())
        elif len(fields) > 1:
            # for each field, get the register, then get it's fields
            # remove duplicate regs - if multiple fields were selected from the same reg
            # do a read optimized on this big list
            # update the fields
            # get the reg dict from the read optimized
            # update the register values
            # print the smaller fields list
            # remove the registers from the regs list if they're already there
            # remove any fields from the list that were read as part of the optimized read
            regs_to_read = []
            for field in fields:
                regs_to_read.append(field.container)
            regs_to_read = list(set(regs_to_read))  # remove duplicates
            for register in regs_to_read:
                reg_val_dict = {}
                fields_read = list(register.get_fields())
                try:
                    field_val_dict = self.current_component.read_fields_optimized(fields_read, reg_val_dict)
                    reg_value_dict.update(reg_val_dict)
                    reg_field_dict[register] = field_val_dict
                    for k, v in reg_val_dict.items():
                        reg_item = self.model.ObjectToItem(k)
                        self.model.ChangeValue(str(v), reg_item, self.value_col_index)  # update the reg in the model
                    for k, v in field_val_dict.items():
                    ...
                    print(traceback.format_exc())
            if print_list is not None:
                component = self.get_objects_component(fields[0])  # just pick a field since user cannot select fields from multiple components
                print_list.append(f'\nfield_list = [')
                for i, field in enumerate(fields):
                    if i == len(fields) - 1:
                        print_list.append(f'\t{component}.{field.path}]')
                    else:
                        print_list.append(f'\t{component}.{field.path},')
                print_list.append(f'field_value_dict = {component}.read_fields_optimized(field_list)')
                for i, field in enumerate(fields):
                    print_list.append(f'field_value_dict[{component}.{field.path}]')
        for reg in regs:  # read the remaining registers
            try:
                ...
                for field in field_value_dict.keys():
                    ...
                    field_item = self.model.ObjectToItem(field)
                    self.model.ChangeValue(str(field_value_dict[field]), field_item, self.value_col_index)  # update the field in the model
                reg_value_dict[reg] = reg_value
                reg_item = self.model.ObjectToItem(reg)
                self.model.ChangeValue(str(reg_value), reg_item, self.value_col_index)  # update the reg in the model
                if print_list is not None:
                    component = self.get_objects_component(reg)
                    print_list.append(f'{component}.{reg.path}.value')
                    pass
            except Exception:
                self.indicate_error()
                print(traceback.format_exc())
    ...
    def write(self, obj, value):
        ...
        if isinstance(obj, Register):
            self.add_recent(text=obj.path)  # add to recent items in search
            reg = obj
            try:
                reg.value = value
                if self.print_writes:
                    if self.write_optimized:
                        ...
                        self.model.ChangeValue(str(value), self.model.ObjectToItem(reg), self.value_col_index)
                        ...
                        for field in list(reg.get_fields()):  # figure out which fields had value changes
                            ...
                            elif field_value != int(self.model.GetValue(self.model.ObjectToItem(field), self.value_col_index), 16):  # field did not have a pending value
                ...
                for field in list(reg.get_fields()):  # calculate the field values and update the model
                    field_value = (value & field.mask) >> field.bit_offset
                    self.model.ChangeValue(str(field_value), self.model.ObjectToItem(field), self.value_col_index)
                if self.write_optimized:
            ...
            if not obj.writeable:  # Log error and dont do transaction
                ...
                try:
                    ...
                    self.model.ChangeValue(str(new_value), self.model.ObjectToItem(reg), self.value_col_index)
                    for field in list(reg.get_fields()):  # calculate the field values and update the model
                ...
                val = self.model.GetValue(self.model.ObjectToItem(reg), self.value_col_index)
                if val == self.model.unknown_value:  # user hasn't read the register yet
                    ...
                    for field in reg.fields.values():
                        self.model.ChangeValue(str(0), self.model.ObjectToItem(field), self.value_col_index)  # update the field in the model
                else:
                ...
                if reg not in self.model.pending_value_dict.keys():
                    ...
                    self.model.ChangeValue(str(reg_value), self.model.ObjectToItem(reg), self.value_col_index)  # change the masked value in the model
                    for k, v in reg.fields.items():  # reset any non-readible fields to 0 so the model is accurate
                        if not v.readable:
                            self.model.ChangeValue(str(0), self.model.ObjectToItem(v), self.value_col_index)  # change the value to in the model
                # Set bits for field write
                masked_reg = reg_value & ~obj.mask
                new_value = masked_reg | ((value << obj.bit_offset) & obj.mask)  # new reg value which includes the new field value

                # Update the GUI elements
                self.model.ChangePendingValue(str(new_value), self.model.ObjectToItem(reg), self.value_col_index)
                for field in list(reg.get_fields()):  # calculate the field values and update the model
                    field_value = (new_value & field.mask) >> field.bit_offset
                    if field is obj:  # this is the field the user set
                        # differentiate the field with pending color
                        self.model.ChangePendingValue(str(field_value), self.model.ObjectToItem(field), self.value_col_index)
    ...
    def handle_event(self, event, evt_type: KeyEventType):
        if evt_type is KeyEventType.ctrl_shift_c:
            ...
            self.set_cursor_busy(busy=True)
            obj_list = self.get_selected_objects()
            if self.print_reads:
                print_list = []
                self.read(obj_list, print_list=print_list)
                print(*print_list, sep='\n')
            else:
                self.read(obj_list)
            for obj in obj_list:
                self.add_recent(text=obj.path if isinstance(obj, Register) or isinstance(obj, Field) else obj.name)  # add item to recent searches
            self.set_cursor_busy(busy=False)
            ...
            if self.field_description.HasFocus():  # user is trying to copy the description text
                ...
                for obj in self.get_selected_objects():  # handle the multi select case
                    ...
                    if isinstance(obj, Block):
                        ...
                        val = self.model.GetValue(self.model.ObjectToItem(obj), self.value_col_index)
            ...
            try:
                comp_path = self.get_objects_component(self.current_component)
                print(f'{comp_path}.reset()')
                self.current_component.reset()
                self.model.Reset()
                for child in self.reg_map_children:
                    self.model.ValueChanged(child, self.value_col_index)
            except Exception:
                self.indicate_error()
                print(traceback.format_exc())
            self.set_cursor_busy(busy=False)
            ...
            if not self.is_mini_mode:
                self.status_bar.SetStatusText('')  # clear out the reg info
    ...
    def indicate_error(self):
        ...
        text = self.status_bar.GetStatusText()  # save current value
        self.status_bar.SetStatusText('*******************************ERROR******************************')
        time.sleep(0.5)
        self.status_bar.SetBackgroundColour(wx.TheColourDatabase.Find('ERROR'))
        self.status_bar.SetStatusText(text)  # restore previous value
