...
from cl_instr_lib.drivers.audio.apx.enums import VerticalAxis, NumberOfGraphPoints, Axis, XUnits, YUnits
...
class MonitorResult(GraphResult):

    """Parent Result Class for all Monitor Results, inherits from Result class"""
    def __init__(self, monitor, result_name):
        self._monitor = monitor
        self.result_name = result_name

    @property
    def api_ref(self):
        return self._monitor.api_ref

    @property
    def _graph_obj(self):
        """Native APX API reference for the graph object"""
        return self.api_ref.__getattribute__(self.result_name)
    ...
    def get_xy_data(self, channel: int = None, vertical_axis: Union[VerticalAxis, int] = VerticalAxis.LEFT,
        ...
        x_data_series = SweepSeries(name=self.configure_axis_names()['x_axis'], data=x_values,
                                    units=self._result_obj.XAxis.Unit, series_type='apx_monitor_results')
        data = [x_data_series]
        if channel is None:
        ...
        for ch in channels:
            ...
            y_label = self.configure_axis_names()['y_axis']
            if add_ch_suffix:
            ...
            y_data_series = SweepSeries(name=y_label, data=y_values, units=self._result_obj.YAxis.Unit,
                                        series_type='apx_monitor_results')
            data.append(y_data_series)
        return data

    def configure_axis_names(self, x_axis: str = None, y_axis: str = None):
        """
        ...
        if x_axis is None and y_axis is None:
            ...
            y_axis_title = self._result_obj.YAxis.Title
        ...
        if y_axis is not None:
            self._result_obj.YAxis.Title = y_axis
    ...
    def get_available_units(self, axis: Union[Axis, str]) -> List[str]:
        """
    ...
    def configure_units(self, x_unit: Union[XUnits, str] = None, y_unit: Union[YUnits, str] = None) -> \
        ...
        :param x_unit:X units options.. all may not be valid depending on input connector settings, but the function
            will check validity for trying to set the configuration. Options include S, HZ, DHZ, F_R, PERCENT_HZ
        ...
        :return: a Dictionary of the (x_unit, y_unit) if no parameters are entered, otherwise returns nothing
