...
class NiPxie4145(InstrumentNiPxiDcpower, PowerSupply):
    """Class for controlling a National Instruments PXIe-4145 4-Channel, ±6 V, 500 mA Precision PXI Source Measure Unit.
    """

    def __init__(self, resource_name=None, rm_handle=None, logger=None, **kwargs):

        model = 'NI PXIe-4145'  # model also serves to identify the model to simulate when resource_name=None.

        super().__init__(model, resource_name, rm_handle, logger, independent_channels=True, **kwargs)

        # No model variants / chassis cooling capacity dependencies to deal with on 4145.
        # Note: Power sinking limit is 7W per module, so it actually depends on what all 4 channels are doing.
        power_source_max = 6 * 0.5
        power_sink_max = 7.0 / 4.0
        # Warn user if they set up any of the 4 channels to exceed one-fourth of module max. This corresponds to the
        # shaded regions in the spec quadrant diagram.
        self.vsource_sink_current_limit = lambda voltage: copysign(0.5, -1.0*voltage) if abs(voltage) <= 3.5 \
            else copysign(power_sink_max/voltage, -1.0*voltage)

        self.isource_sink_voltage_limit = lambda current: copysign(6.0, -1.0*current) if \
            abs(current) <= (power_sink_max/6.0) else copysign(power_sink_max/current, -1.0*current)

        programing_bounds = ProgrammingBounds(-6.0, 6.0, -0.5, 0.5, -1.0*power_sink_max, power_source_max)
        ...
        self.compensation_bounds = CompensationBounds(gain_bandwidth_voltage=min_max_units(100, 1.6e6, 'Hz'),
                                                      gain_bandwidth_current=min_max_units(1e3, 400e3, 'Hz'),
        ...
        self.CHANNEL0 = NiSmuChannel(self, "CHANNEL0", 0, programing_bounds)
        self.CHANNEL1 = NiSmuChannel(self, "CHANNEL1", 1, programing_bounds)
        self.CHANNEL2 = NiSmuChannel(self, "CHANNEL2", 2, programing_bounds)
        self.CHANNEL3 = NiSmuChannel(self, "CHANNEL3", 3, programing_bounds)

        self._channel_names = ["CHANNEL0", "CHANNEL1", "CHANNEL2", "CHANNEL3"]


if __name__ == '__main__':
    smu4145 = NiPxie4145('PXI1Slot6')  # None)  # Replace resource_name with None for simulated hardware.
