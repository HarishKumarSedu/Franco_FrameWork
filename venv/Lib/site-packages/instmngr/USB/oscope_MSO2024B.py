...
class Oscope_MSO2024B():
    ...
    instDict = {'discrete': {'ch1state': ['ON', {'ON': 'ON', 'OFF': 'OFF'}, 'SEL:CH1', False],
                             ...
                             'ch1bandwidth': ['FULL', {'20MHz': 'TWE', '250MHz': 'TWO', 'FULL': 'FULL'}, 'CH1:BAN',
                                              False],
                             'ch2bandwidth': ['FULL', {'20MHz': 'TWE', '250MHz': 'TWO', 'FULL': 'FULL'}, 'CH2:BAN',
                                              False],
                             'ch3bandwidth': ['FULL', {'20MHz': 'TWE', '250MHz': 'TWO', 'FULL': 'FULL'}, 'CH3:BAN',
                                              False],
                             'ch4bandwidth': ['FULL', {'20MHz': 'TWE', '250MHz': 'TWO', 'FULL': 'FULL'}, 'CH4:BAN',
                                              False],
                             ...
                             'recordLength': ['10000', {'1k': '1000', '10k': '10000', '100k': '100000', '1M': '1000000',
                                                        '10M': '10000000'}, 'HOR:RECO', False],
                             'horizDelayMode': ['ON', {'ON': 'ON', 'OFF': 'OFF'}, 'HOR:DEL:MOD', False],
                             'triggerType': ['EDG', {'edge': 'EDG', 'logic': 'LOGI', 'pulse': 'PUL'}, 'TRIG:A:TYP',
                                             False],
                             'triggerMode': ['AUTO', {'auto': 'AUTO', 'normal': 'NORM'}, 'TRIG:A:MOD', False],
                             'triggerEdgeSource': ['CH1', {'ch1': 'CH1', 'ch2': 'CH2', 'ch3': 'CH3', 'ch4': 'CH4'},
                                                   'TRIG:A:EDGE:SOU', False],
    ...
    def __init__(self, rmHandle):
        ...
        self.logger = logging.getLogger('marv.inst.Oscope_MSO2024B')
        self.logger.info('Initializing Oscope_MSO2024B instrument')
    ...
    def setRiseFallLevel(self, voh=0.7, vol=0.3):
        self.rmHandle.write('MEASUREMENT:REFLEVEL:METHOD')
    ...
    def setMidLevel(self, srcLvl=0.5, destLvl=0.5):
        self.rmHandle.write('MEASUREMENT:REFLEVEL:METHOD')
        self.rmHandle.write('MEASUREMENT:REFLEVEL:PERCENT:MID ' + str(srcLvl * 100))
    ...
    def getScreenShot(self, filename=''):
        ...
        if len(filename):
            ...
            newfile.close()
            return len(
                result)  # When writing to a file just return the number of bytes written, so as not to clobber an interactive session
    ...
    def getWfmData(self, source='Ch1', data_size=2):
        ...
        if source[1] == 'D':
            ...
            ymult = float(self.rmHandle.query('WFMOutpre:YMUlt?'))  # waveform vertical scale factor per digitizing level
            yoff = float(self.rmHandle.query('WFMOutpre:YOFF?'))  # vertical position in digitizing levels
            ydata = [ymult * (out - yoff) for out in int_data]  # converts waveform sample values to Volts

        xmult = float(self.rmHandle.query('WFMOutpre:XINCR?'))  # horizontal point spacing
        xoff = float(self.rmHandle.query(
            'WFMOutpre:PT_OFF?'))  # always returns a 0 - this command is only for compatibility with other Tek scopes
        xzero = float(self.rmHandle.query('WFMOutpre:XZERO?'))  # time coordinate of the first point in the outgoing waveform
...
if __name__ == "__main__":
    ...
    #scope = instMgr.checkout(interface='USB', addr='C040317', moduleSuffix='MSO2024B')
    scope = instMgr.checkout(interface='USB', addr='C012764', moduleSuffix='MSO2024B')
    scope.reset()
    print(scope.getIdn())
